...
The production follow-up confirmed owner API responses from memory (Daniel 75 ms; Marita 1,043 ms for the full payload; unchanged responses 69–524 ms). These are individual runner observations, not page-load guarantees. The management probe exposed a different default period: runtime configuration can point to a historical range while the client/comparison defaults to the current month. Warmup now prioritizes the current month for both owners, then also warms any configured historical start date. All timing probes use the same explicit monthly period.
