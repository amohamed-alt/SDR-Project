...
try {
  await waitForReady();
  const [healthResponse, dashboardResponse, pageResponse] = await Promise.all([
    fetch(`http://127.0.0.1:${port}/api/health`),
    fetch(`http://127.0.0.1:${port}/api/dashboard?from=2026-07-01&to=2026-07-19&ownerId=31644369`),
    fetch(`http://127.0.0.1:${port}/`),
  ]);
  if (!healthResponse.ok || !dashboardResponse.ok || !pageResponse.ok) throw new Error("One or more smoke-test routes returned an error");
