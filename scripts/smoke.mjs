...
try {
  await waitForReady();
  const [healthResponse, dashboardResponse, acquisitionResponse, calendarStatusResponse, rejectedMeetingResponse, pageResponse] = await Promise.all([
  ...
  if (!healthResponse.ok || !dashboardResponse.ok || !acquisitionResponse.ok || !calendarStatusResponse.ok || !pageResponse.ok) throw new Error("One or more smoke-test routes returned an error");
  ...
  if (!acquisition.team || acquisition.meta?.isDemo !== true) throw new Error("Acquisition response is invalid");
  ...
  console.log("Smoke tests passed: page, health API, SDR API, and Acquisition API are operational.");
} finally {
  server.kill("SIGTERM");
}
