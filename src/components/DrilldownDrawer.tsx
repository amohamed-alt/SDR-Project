...
function ActivityCard({ row }: { row: ActivityRow }) {
  return <article className="drawer-record-card">
    ...
    <div className="drawer-record-actions">{row.type === "Task" && row.relatedContactId && <WhatsAppQuickAction contactId={row.relatedContactId}/>} {externalLink(row.url, row.relatedContactUrl ? "Open contact timeline in HubSpot" : "Open activity list in HubSpot")}</div>
  </article>;
}
