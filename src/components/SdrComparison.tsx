...
export function SdrComparison({ onSelect }: { onSelect: (key: SdrKey) => void }) {
  ...
  const chart = metrics.filter(metric => ["calls", "connectedCalls", "bookedMeetings", "completedTasks"].includes(metric.key)).map(metric => ({
    name: metric.label, Marita: dataFor("marita")?.data?.kpis[metric.key] ?? null, Daniel: dataFor("daniel")?.data?.kpis[metric.key] ?? null,
  }));
  return <main className={styles.page}>
    ...
    <section className={styles.title}><div><p>MANAGEMENT OVERVIEW</p><h1>SDR performance</h1><span>Marita · Talentera &nbsp; / &nbsp; Daniel · Evalufy</span></div><form onSubmit={event => { event.preventDefault(); setRange({ ...draft }); }}><CalendarDays size={18}/><label>From<input aria-label="Reporting start" type="date" required value={draft.from} max={draft.to} onChange={event => setDraft({ ...draft, from: event.target.value })}/></label><label>To<input aria-label="Reporting end" type="date" required min={draft.from} value={draft.to} onChange={event => setDraft({ ...draft, to: event.target.value })}/></label><button>Apply</button></form></section>
    ...
    <div className={styles.detail}><section className={styles.panel}><h2>Activity comparison</h2><p>Same reporting period · recorded HubSpot activity</p><ResponsiveContainer width="100%" height={320}><BarChart data={chart} margin={{ top: 20, right: 12, left: -15, bottom: 0 }}><CartesianGrid vertical={false} stroke="#e8ebef"/><XAxis dataKey="name" tick={{ fontSize: 12 }} interval={0} height={55}/><YAxis allowDecimals={false}/><Tooltip/><Legend/><Bar dataKey="Marita" fill={SDR_OWNERS.marita.color} radius={[4,4,0,0]} isAnimationActive={false}/><Bar dataKey="Daniel" fill={SDR_OWNERS.daniel.color} radius={[4,4,0,0]} isAnimationActive={false}/></BarChart></ResponsiveContainer></section>
    <section className={styles.panel}><h2>Numbers at a glance</h2><div className={styles.tableWrap}><table><caption className={styles.caption}>Activity dates: {range.from} – {range.to}. Workload and portfolio are current snapshots.</caption><thead><tr><th scope="col">Metric</th><th scope="col">Marita</th><th scope="col">Daniel</th></tr></thead><tbody>{metrics.map(metric => <tr key={metric.key}><th scope="row">{metric.label}</th>{(["marita", "daniel"] as const).map(owner => <td key={owner}>{dataFor(owner)?.data ? `${format(dataFor(owner)!.data!.kpis[metric.key])}${metric.percent ? "%" : ""}` : "—"}</td>)}</tr>)}</tbody></table></div></section></div>
