...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!item || item.status !== "success" || !item.candidate) {
      return NextResponse.json({ error: "SignalHire could not resolve this LinkedIn profile.", creditsLeft }, { status: 404 });
    }
    ...
    return NextResponse.json({
      prospect: {
        ...
        companyWebsite: currentRole?.website && currentRole.website !== "n/a" ? currentRole.website : "",
    ...
    console.error("Prospecting SignalHire analysis failed", error);
