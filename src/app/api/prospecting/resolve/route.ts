...
function normalizeLinkedInUrl(raw: string) {
  ...
  if (!/^\/in\//i.test(url.pathname)) {
    throw new Error("Only LinkedIn person profile URLs are supported.");
  }
  url.protocol = "https:";
...
function recentRoleSignal(current: SignalHireExperience | undefined, previous: SignalHireExperience | undefined) {
  ...
  if (ageDays === null || ageDays > 120 || !current?.company) {
    return { type: "", label: "", ageDays };
  }
  const sameCompany = Boolean(
    previous?.company && previous.company.trim().toLowerCase() === current.company.trim().toLowerCase(),
  );
  if (sameCompany && previous?.position && previous.position !== current.position) {
    return { type: "role_change", label: `Changed role ${ageDays} days ago`, ageDays };
  }
  if (previous?.company && !sameCompany) {
    return { type: "job_change", label: `Changed company ${ageDays} days ago`, ageDays };
  }
...
function emptyHiring() {
  return {
    ...
    activeJobs: 0,
    hiringScore: 0,
    hiringLabel: "Checking",
    hasHrJobs: false,
    source: "",
    sourceUrl: "",
    checkedAt: "",
    jobsSample: [] as Array<{ title: string; location: string; url: string }>,
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json({ error: "SIGNALHIRE_API_KEY is not configured on the server." }, { status: 503 });
    }
    ...
    if (!response.ok) {
      const message = payload && !Array.isArray(payload) && payload.error
        ? payload.error
        : `SignalHire returned HTTP ${response.status}.`;
      return NextResponse.json(
        { error: message, creditsLeft },
    ...
    if (!item || item.status !== "success" || !item.candidate) {
      return NextResponse.json(
        { error: "SignalHire could not resolve this LinkedIn profile.", creditsLeft },
        { status: 404 },
      );
    }
    ...
    const scoreReasons: Array<{ label: string; points: number }> = [
      { label: "SignalHire profile resolved", points: 15 },
    ];

    const persona = personaScore(title);
    if (persona) {
      score += persona;
      scoreReasons.push({ label: "Relevant HR / Talent persona", points: persona });
    }
    const geo = geographyScore(location);
    if (geo) {
      score += geo;
      scoreReasons.push({ label: "Priority geography", points: geo });
    }
    const companySize = companySizeScore(currentRole);
    if (companySize) {
      score += companySize;
      scoreReasons.push({ label: "Company size fit", points: companySize });
    }
    if (recentSignal.type) {
      score += 20;
      scoreReasons.push({ label: recentSignal.label, points: 20 });
    }
    if (email?.value) {
      score += 7;
      scoreReasons.push({ label: "Email available", points: 7 });
    }
    if (phone?.value) {
      score += 3;
      scoreReasons.push({ label: "Phone available", points: 3 });
    }
    ...
    console.error("Fast prospect resolution failed", error);
    if (error instanceof DOMException && error.name === "TimeoutError") {
      return NextResponse.json(
        { error: "SignalHire took longer than 30 seconds. Try this lead again." },
        { status: 504 },
      );
    }
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Prospect resolution failed." },
      { status: 500 },
    );
  }
}
